...
INTERVAL_REPORT_HEARTBEAT = 2  # 每隔若干秒向 LoadBalancer 报告自己的状态（正在和即将处理的请求数量）
SLAVE_API = "/generate"
    ...
    async def stream_results() -> AsyncGenerator[bytes, None]:
        ...
        while True:
            for i, stream in enumerate(streams):
                ...
                rets[i] = {
                    "req_id": req_id, "text": text_output, "finished": finished[i]
                }
...
def get_local_addr():
    global RUNNING_HOST, RUNNING_PORT, SLAVE_API
    return f"http://{RUNNING_HOST}:{RUNNING_PORT}{SLAVE_API}"
...
async def report_heartbeat():
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            await asyncio.sleep(INTERVAL_REPORT_HEARTBEAT)
...
if __name__ == "__main__":
    ...
    with open(config_file, 'r', encoding='utf-8') as f:
        config = yaml.safe_load(f)
    INTERVAL_REPORT_HEARTBEAT = config["APIServer"]["intervalReportHeartbeat"]
    
    RUNNING_HOST = args.host if args.host is not None else "127.0.0.1"
