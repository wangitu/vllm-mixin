...
host = "127.0.0.1"
port = "8080"
heartbeat_api = "/stats/report-heartbeat" # 接收 APIServer 报告状态的接口
api_urls = {} # 从节点的 api: num_request

concurrency = 50  # 并发处理 queries 的数量
wait_period = 1  # 每若干秒处理一批 queries
...
class QueryProcessor:
    ...
    async def process(queries: List[Tuple]):
        ...
        async with aiohttp.ClientSession() as session:
            response = await session.post(api_url, json=pload)
            data_buffer = ""
            async for chunk in response.content.iter_chunked(8192):
                if chunk:
                    data_buffer += chunk.decode("utf-8")
                    if data_buffer.endswith("\x00"):
                        data = json.loads(data_buffer.rstrip("\x00"))
                        data_buffer = ""
                        for ret in data:
...
class QueryRequest(BaseModel):
    ...
    decoding_args: Optional[DecodingArgs] = Field(default_factory=DecodingArgs)
...
async def generate(request: QueryRequest):
    ...
    decoding_args = request.decoding_args.dict()
        ...
        if not isinstance(history, list) or \
            (len(history) > 0 and \
                (not isinstance(history[0], (list, tuple)) or \
                    len(history[0]) != 2 or not isinstance(history[0][0], str) or not isinstance(history[0][1], str))):
            return Response("`history` must be a JSON string of List[(query, answer)]", status_code=400)
    ...
    await queries_queue.put((query, history, decoding_args, result_queue))
...
if __name__ == "__main__":
    ...
    concurrency = load_balancer["concurrency"]
    wait_period = load_balancer["waitPeriod"]
